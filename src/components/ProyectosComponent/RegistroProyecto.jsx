...
const RegistroProyecto = () => {
  ...
  const RedireccionarInvestigador = (correo) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await registroProyecto(formData);
      console.log(formData);
      alert('Proyecto añadido correctamente');

    } catch (error) {
      console.error('Error al añadir proyecto', error);
      alert('Error al añadir proyecto. Por favor, intentalo de nuevo')
        ...
        <div className=" rounded-md p-4 relative  border shadow-2xl bg-gray-800 border-gray-700   shadow-blue-500/50  ">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div class="flex flex-col items-center justify-center space-y-6">
              <input type="text" id="titulo_esp" name="titulo_esp" placeholder="Titulo" class="w-96 appearance-none rounded-full border-0 bg-slate-700 p-2 px-4 focus:bg-slate-800 focus:ring-2 focus:ring-orange-500" />
              ...
              <button id="showPw" class="rounded-full bg-indigo-500 p-2 px-4 text-white hover:bg-indigo-700"><span id="showHide">Añadir</span> Proyecto</button>
